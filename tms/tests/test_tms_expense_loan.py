...
class TestTmsExpenseLoan(TransactionCase):
    ...
    def test_10_tms_expense_loan_create(self):
        ...
        with self.assertRaisesRegex(ValidationError, "You need to define the sequence for loans in base Mexico"):
    ...
    def test_30_tms_expense_loan_action_approve(self):
        loan = self.create_expense_loan()
        msg = "Could not approve the Loan. The Amount of discount must be greater than zero."
    ...
    def test_40_tms_expense_loan_action_cancel(self):
        ...
        loan.action_confirm()
        wizard = Form(
            self.env["account.payment.register"].with_context(active_model="tms.expense.loan", active_ids=loan.ids)
        ).save()
    ...
    def test_52_tms_expense_loan_action_confirm(self):
        ...
        with self.assertRaisesRegex(ValidationError, "Warning! You must have configured the accounts of the tms"):
            loan.action_confirm()
    ...
    def test_60_tms_expense_loan_unlink(self):
        ...
        loan.action_approve()
        loan.action_confirm()
        with self.assertRaisesRegex(ValidationError, "You can not delete a Loan in status confirmed or closed"):
            loan.unlink()
        loan.action_cancel()
