...
class TestTmsRoute(TransactionCase):
    def setUp(self):
        ...
        self.result = {
            ...
            "origin_addresses": ["501-599 E Mills Ave, El Paso, TX 79901, USA"],
            "destination_addresses": ["206 N Guadalupe St, San Marcos, TX 78666, USA"],
        }
    ...
    def test_10_tms_route_on_change_distance_empty(self):
        ...
        self.assertEqual(self.route.distance_loaded, 737.00, "On change works")
        with self.assertRaisesRegex(ValidationError, "The value must be positive and lower than the distance route."):
    ...
    def test_20_tms_route_on_change_distance_loaded(self):
        ...
        self.assertEqual(self.route.distance_empty, 737.00, "On change works")
        with self.assertRaisesRegex(ValidationError, "The value must be positive and lower than the distance route."):
    ...
    def test_30_tms_route_get_route_info(self):
        ...
        self.assertEqual(self.route.travel_time, 8.196100000000001, msg="Travel time is not correct")
        self.assertEqual(self.route.distance, 946.8870000000001, msg="Distance is not correct")
        json.loads.return_value = False
        with self.assertRaisesRegex(UserError, "Google Maps is not available."):
            self.route.get_route_info()

    def test_31_tms_route_get_route_info(self):
        with self.assertRaisesRegex(UserError, "The arrival don't have coordinates."):
            ...
            self.route.get_route_info()
        with self.assertRaisesRegex(UserError, "The departure don't have coordinates."):
