...
class TestTmsAdvance(TransactionCase):
    ...
    def test_10_tms_advance_create(self):
        with self.assertRaisesRegex(ValidationError, "The amount must be greater than zero."):
            self.create_advance(0.0)
        self.operating_unit.advance_sequence_id = False
        with self.assertRaisesRegex(ValidationError, "The sequence is not defined in operating unit Mexico"):
    ...
    def create_advance_cancel(self):
        ...
        advance.action_confirm()
        wizard = Form(
            self.env["account.payment.register"].with_context(active_model="tms.advance", active_ids=advance.ids)
        ).save()
    ...
    def test_70_tms_advance_action_cancel_draft(self):
        ...
        with self.assertRaisesRegex(
            ValidationError, "Could not set this advance to draft because the travel is cancelled."
        ):
