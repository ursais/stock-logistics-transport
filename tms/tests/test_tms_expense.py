...
class TestTmsExpense(TransactionCase):
    def setUp(self):
        ...
        self.operating_unit = self.env.ref("operating_unit.main_operating_unit")
        ...
        wizard = Form(
            self.env["account.payment.register"].with_context(
                active_model="tms.advance", active_ids=self.waybill.travel_ids.mapped("advance_ids").ids
            )
        ).save()
    ...
    def test_10_tms_expense_create_advance_line(self):
        ...
        with self.assertRaises(ValidationError) as err:
            self.create_expense()
        self.waybill.travel_ids.mapped("advance_ids").filtered(lambda x: x.state == "closed").write(
            {"state": "confirmed"}
        )
        self.assertEqual(
        ...
        with self.assertRaises(ValidationError) as err2:
            self.create_expense()
        self.assertEqual(
            err2.exception.args[0], "Oops! All the advances must be paid\n Name of advance not paid: " + adv.name
        )
    ...
    def test_30_tms_expense_create(self):
        ...
        self.assertEqual(
            len(expense.expense_line_ids.filtered(lambda x: x.line_type == "salary")), len(expense.travel_ids)
        )

        # Salary
        amount_salary = len(self.waybill.travel_ids) * sum(self.waybill.driver_factor_ids.mapped("fixed_amount"))
        ...
        # Advances
        amount_advance = sum(
            self.waybill.travel_ids.mapped("advance_ids").filtered(lambda x: x.payment_move_id).mapped("amount")
        )
    ...
    def test_40_tms_expense_action_confirm(self):
        ...
        fuel_line_ids = expense.fuel_log_ids.filtered(lambda x: x.created_from_expense).mapped("expense_line_id")
        line_ids = expense.expense_line_ids.filtered(lambda x: x.expense_fuel_log).mapped("id")
        for lid in line_ids:
    ...
    def test_50_tms_expense_action_cancel(self):
        ...
        if expense.expense_line_ids.filtered(lambda x: x.expense_fuel_log):
            self.assertFalse(any(expense.fuel_log_ids.filtered(lambda x: x.created_from_expense)))
