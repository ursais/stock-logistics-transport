...
class TestFleetVehicleLogFuel(TransactionCase):
    ...
    def test_20_fleet_vehicle_log_fuel_create(self):
        ...
        with self.assertRaisesRegex(ValidationError, "You need to define the sequence for fuel logs in base Mexico"):
    ...
    def test_30_fleet_vehicle_log_fuel_action_cancel(self):
        ...
        with self.assertRaisesRegex(
            ValidationError,
            "Could not cancel Fuel Voucher! This Fuel Voucher is already linked to a Travel Expense",
        ):
            log_fuel.action_cancel()
        log_fuel.action_confirm()
        wizard = (
            self.env["tms.wizard.invoice"]
            .with_context(**{"active_model": "fleet.vehicle.log.fuel", "active_ids": [log_fuel.id]})
            .create({})
        )
        wizard.make_invoices()
        with self.assertRaisesRegex(
            ValidationError, "Could not cancel Fuel Voucher! This Fuel Voucher is already Invoiced"
        ):
            log_fuel.action_cancel()
