...
class TestHrEmployee(TransactionCase):
    ...
    def test_20_get_driver_license_info(self):
        ...
        self.assertEqual(self.employee_id.license_valid_from.strftime("%Y-%m-%d"), "2012-06-26")
        self.assertEqual(self.employee_id.license_expiration.strftime("%Y-%m-%d"), "2017-06-26")
        Socrata.get.return_value = False
        with self.assertRaisesRegex(ValidationError, "The driver license is not in SCT database"):
            self.employee_id.get_driver_license_info()
