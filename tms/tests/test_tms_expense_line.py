...
class TestTmsExpenseLine(TransactionCase):
    def setUp(self):
        ...
        self.driver.write(
            {
                "address_home_id": self.env.ref("base.res_partner_address_31").id,
                "tms_advance_account_id": advance.id,
                "tms_expense_negative_account_id": balance.id,
            }
        )
        self.advance_1 = self.env.ref("tms.tms_advance_01").state = "confirmed"
        self.advance_2 = self.env.ref("tms.tms_advance_02").state = "confirmed"
    ...
    def test_30_tms_expense_line_create(self):
        expense = self.create_expense()
        with self.assertRaisesRegex(ValidationError, "This line type needs a negative value to continue!"):
