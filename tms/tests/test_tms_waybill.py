...
class TestTmsWaybill(TransactionCase):
    ...
    def test_10_tms_waybill_onchange_partner_id(self):
        ...
        address = self.customer3.address_get(["invoice", "contact"]).get("contact", False)
    ...
    def create_waybill_invoice_paid(self):
        ...
        wizard = (
            self.env["tms.wizard.invoice"]
            .with_context(**{"active_model": "tms.waybill", "active_ids": [waybill.id]})
            .create({})
        )
        ...
        payment.action_post()
        invoice = json.loads(waybill.invoice_id.invoice_outstanding_credits_debits_widget)
        waybill.invoice_id.js_assign_outstanding_line(invoice["content"][0]["id"])
        return waybill
    ...
    def test_40_tms_waybill_amount(self):
        ...
        for product in products:
            product_id = self.env["product.product"].search([("tms_product_category", "=", product[0])])
    ...
    def test_50_tms_waybill_action_confirm(self):
        ...
        with self.assertRaisesRegex(
            ValidationError, "Could not confirm Waybill! Waybill must be assigned to a Travel before confirming."
        ):
            waybill.action_confirm()
    ...
    def test_70_tms_waybill_action_cancel_draft(self):
        ...
        with self.assertRaisesRegex(ValidationError, "Could not set to draft this Waybill! Travel is Cancelled !!!"):
    ...
    def test_90_tms_waybill_transportable_product(self):
        waybill = self.create_waybill()
        waybill.transportable_line_ids.transportable_id = self.transportable2.id
    ...
    def test_100_tms_waybill_action_cancel(self):
        ...
        with self.assertRaisesRegex(
            ValidationError,
            "You cannot unlink the invoice of this waybill because the invoice is still valid, please check it.",
        ):
            waybill.action_cancel()
