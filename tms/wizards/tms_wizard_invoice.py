...
class TmsWizardInvoice(models.TransientModel):
    ...
    @api.model
    def prepare_lines(self, product, quantity, price_unit, tax, account, origin):
        return {
    ...
    def compute_waybill(self, record, lines):
        ...
        res["fpos"] = fpos
        res["invoice_account"] = fpos.map_account(partner_id.property_account_receivable_id)
        for line in record.waybill_line_ids:
            if line.product_id.property_account_income_id:
                account = fpos.map_account(line.product_id.property_account_income_id)
            elif line.product_id.categ_id.property_account_income_categ_id:
                account = fpos.map_account(line.product_id.categ_id.property_account_income_categ_id)
            else:
                raise exceptions.ValidationError(_("You must have an income account in the product or its category."))
            tax = fpos.map_tax(line.tax_ids)
            if line.price_subtotal > 0.0:
                lines.append(
                    (
                        ...
                        0,
                        self.prepare_lines(
                            line.product_id, line.product_qty, line.price_subtotal, tax, account, record.name
                        ),
                    )
                )
        res["lines"] = lines
    ...
    def compute_fuel_log(self, record, lines):
        ...
        fpos = partner_id.property_account_position_id
        res["fpos"] = fpos
        res["invoice_account"] = fpos.map_account(partner_id.property_account_payable_id)
        ieps = record.operating_unit_id.ieps_product_id
        products = [record.product_id, ieps]
        for product in products:
            account = product.product_tmpl_id.get_product_accounts(fpos).get("expense", False)
            if not account:
                raise exceptions.ValidationError(_("You must have an expense account in the product or its category"))
            ...
            if product.id == record.product_id.id:
                lines.append(
                    (
                        0,
                        0,
                        self.prepare_lines(product, record.product_qty, record.price_unit, tax, account, record.name),
                    )
                )
                ...
                lines.append(
                    (
                        ...
                        0,
                        self.prepare_lines(
                            record.operating_unit_id.ieps_product_id,
                            1.0,
                            record.special_tax_amount,
                            tax,
                            account,
                            record.name,
                        ),
    ...
    def make_invoices(self):
        ...
        for record in records:
            if record.invoice_id:
                raise exceptions.ValidationError(_("The record is already invoiced"))
            if record.state not in ["confirmed", "closed"]:
                raise exceptions.ValidationError(_("The record must be confirmed or closed"))
        ...
        if len(set(partner_ids)) > 1:
            raise exceptions.ValidationError(_("The records must be of the same partner."))
        if len(set(currency_ids)) > 1:
            raise exceptions.ValidationError(_("The records must be of the same currency."))
        ...
        message = _("<strong>Invoice of:</strong> %(name)s </br>", name=", ".join(record_names))
