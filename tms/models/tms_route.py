...
class TmsRoute(models.Model):
    ...
    fuel_efficiency_ids = fields.One2many("tms.route.fuelefficiency", "route_id", string="Fuel Efficiency")
    route_place_ids = fields.One2many("tms.route.place", "route_id", string="Places")
    tollstation_ids = fields.Many2many("tms.route.tollstation", string="Toll Station")
    note_ids = fields.One2many("tms.route.note", "route_id", string="Notes of Route")
    ...
    def get_route_info(self, error=False):
        for rec in self:
            departure = {"latitude": rec.departure_id.latitude, "longitude": rec.departure_id.longitude}
            arrival = {"latitude": rec.arrival_id.latitude, "longitude": rec.arrival_id.longitude}
            if not departure["latitude"] and not departure["longitude"]:
                raise exceptions.UserError(_("The departure don't have coordinates."))
            if not arrival["latitude"] and not arrival["longitude"]:
                raise exceptions.UserError(_("The arrival don't have coordinates."))
            url = "http://maps.googleapis.com/maps/api/distancematrix/json"
            origins = str(departure["latitude"]) + "," + str(departure["longitude"])
            destinations = ""
            places = [
                str(x.place_id.latitude) + "," + str(x.place_id.longitude)
                for x in rec.route_place_ids
            ...
            try:
                result = json.loads(requests.get(url, params=params, timeout=120).content)
                distance = duration = 0.0
                if result["status"] == "OK":
                    if rec.route_place_ids or (rec.departure_id and rec.arrival_id):
                        pos = 0
                        for row in result["rows"]:
                            distance += row["elements"][pos]["distance"]["value"] / 1000.0
                            duration += row["elements"][pos]["duration"]["value"] / 3600.0
                            pos += 1
    ...
    def get_fuel_efficiency(self, vehicle_id, framework):
        for rec in self:
            ...
            fuel_id = fuel.search(
                [("route_id", "=", rec.id), ("engine_id", "=", vehicle_id.engine_id.id), ("type", "=", framework)]
            )
