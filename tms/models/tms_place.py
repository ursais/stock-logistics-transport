...
class TmsPlace(models.Model):
    ...
    def get_coordinates(self):
        for rec in self:
            if rec.name and rec.state_id:
                address = rec.name + "," + rec.state_id.name + "," + rec.state_id.country_id.name
            else:
                raise ValidationError(_("You need to set a Place and a State Name"))
            key = self.env["ir.config_parameter"].get_param("mapquest.key")
            if key == "key":
                raise ValidationError(_("You need to define mapquest.key parameter."))
            params = {
                "key": key,
                "outFormat": "json",
            ...
            result = requests.get(url, params=params, timeout=120)
    ...
    def open_in_google(self):
        for place in self:
            url = (
                "/tms/static/src/googlemaps/get_place_from_coords.html?"
                + str(place.latitude)
                + ","
                + str(place.longitude)
            )
