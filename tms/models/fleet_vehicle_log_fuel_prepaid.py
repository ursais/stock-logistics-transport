...
class FleetVehicleLogFuelPrepaid(models.Model):
    ...
    invoice_paid = fields.Boolean(compute="_compute_invoiced_paid")
    operating_unit_id = fields.Many2one("operating.unit")
    notes = fields.Char()
    state = fields.Selection(
        [("draft", "Draft"), ("confirmed", "Confirmed"), ("closed", "Closed")], readonly=True, default="draft"
    )
    vendor_id = fields.Many2one("res.partner", string="Supplier")
    date = fields.Date(required=True, default=fields.Date.context_today)
    product_id = fields.Many2one("product.product", domain=[("tms_product_category", "=", "fuel")])
    ...
    def create(self, values):
        ...
        if not res.operating_unit_id.prepaid_fuel_sequence_id:
            raise ValidationError(
                _(
                    "You need to define the sequence for fuel logs in base %(operating_unit)s",
                    operating_unit=res.operating_unit_id.name,
                )
            )
        ...
        for rec in self:
            ...
            for fuel in rec.log_fuel_ids:
                ...
                if balance > rec.price_total:
                    raise ValidationError(_("The total amount of fuel voucher is higher than the allowed limit"))
            rec.balance = balance
    ...
    def create_invoice(self):
        ...
        for rec in self:
            ...
            if not account["expense"]:
                raise ValidationError(_("You must have an income account in the product or its category."))
            ...
            message = _("<strong>Invoice of:</strong> %(name)s </br>", name=rec.name)
