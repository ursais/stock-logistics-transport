...
class HrEmployee(models.Model):
    ...
    tms_loan_account_id = fields.Many2one("account.account", "Loan Account", company_dependent=True)
    ...
    def get_driver_license_info(self):
        ...
        for rec in self:
            try:
                driver_license = client.get("3qhi-59v6", licencia=rec.driver_license)
                license_valid_from = datetime.strptime(
                    driver_license[0]["fecha_inicio_vigencia"], "%Y-%m-%dT%H:%M:%S.%f"
                )
                license_expiration = datetime.strptime(driver_license[0]["fecha_fin_vigencia"], "%Y-%m-%dT%H:%M:%S.%f")
                rec.write(
                    {
                        "license_type": driver_license[0]["categoria_de_la_licencia"],
                ...
                client.close()
                raise ValidationError(_("The driver license is not in SCT database"))
