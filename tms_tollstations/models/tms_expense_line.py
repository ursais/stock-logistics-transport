...
class TmsExpenseLine(models.Model):
    ...
    def create(self, values):
        ...
        res.tollstation_ids.write({"state": "closed", "expense_line_id": res.id})
        return res

    def write(self, values):
        for rec in self:
            res = super().write(values)
            tolls = self.env["tms.toll.data"].search([("expense_line_id", "=", self.id)])
            tolls.write({"expense_line_id": False, "state": "closed"})
            for toll in rec.tollstation_ids:
                toll.write({"state": "closed", "expense_line_id": rec.id})
        return res

    def unlink(self):
        for rec in self:
            tolls = self.env["tms.toll.data"].search([("expense_line_id", "=", rec.id)])
            tolls.write({"state": "open", "expense_line_id": False})
        return super().unlink()

    def sort_expense_lines(self):
        for rec in self:
            ordered_lines = sorted(
                rec.tollstation_ids, key=lambda x: datetime.strptime(x["date"], "%Y-%m-%d %H:%M:%S")
            )
