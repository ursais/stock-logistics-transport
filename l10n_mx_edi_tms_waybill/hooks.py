...
def post_init_hook(cr, registry):
    ...
    if not env["l10n_mx_edi.station"].search_count([]):
        ...
        with open(csv_path, "r") as csv_file:
            for row in csv.DictReader(
                csv_file, delimiter="|", fieldnames=["l10n_mx_edi_transport_type", "code", "name", "country_xml_id"]
            ):
    ...
    if not env["l10n_mx_edi.dangerous.material"].search_count([]):
        ...
        material_vals_list = []
        with open(csv_path, "r") as csv_file:
            for row in csv.DictReader(csv_file, delimiter="|", fieldnames=["code", "name"]):
        ...
        product_dict = {}
        with open(csv_path, "r") as csv_file:
            for row in csv.DictReader(csv_file, delimiter="|", fieldnames=["code", "type"]):
